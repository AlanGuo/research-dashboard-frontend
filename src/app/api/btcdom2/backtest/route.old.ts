...
class BTCDOM2StrategyEngine {
  ...
  async generateSnapshot(
    ...
    if (isActive) {
      ...
      if (canShortAlt && hasShortCandidates) {
        ...
        if (previousSnapshot?.shortPositions) {
          ...
          if (previousShortPosition) {
            // 确保之前的持仓数据有效
            const validPrevAmount = previousShortPosition.value ?? 0;
      ...
      const validPrevBtcAmount = previousBtcPosition.value ?? btcAmount;
      ...
      account_usdt_balance = totalValue - (btcPosition ? btcPosition.value : 0);
    }
