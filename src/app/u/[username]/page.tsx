...
export default function UserPage() {
  ...
  const fetchBtcData = async (startDate: string, endDate: string) => {
    try {
      ...
      const apiBaseUrl = config.api?.baseUrl || 'http://localhost:3001/api';
      const response = await fetch(`${apiBaseUrl}/kline/btcusdt?interval=1D&bars=${bars}`);
  ...
  useEffect(() => {
    async function fetchFundRealtimeData() {
      try {
        ...
        const apiBaseUrl = config.api?.baseUrl || 'http://localhost:3001/api';
        try {
          const holdingResponse = await fetch(`${apiBaseUrl}/holding/${username}?status=持仓`);
          ...
          console.error('Error fetching holding strategies:', holdingErr);
        }
  ...
  return (
    <div className="container mx-auto py-6 md:py-10 px-4 md:px-6 space-y-6 md:space-y-8">
      ...
      {/* 持仓策略模块 */}
      {!loading && !error && baseInfoResponse && baseInfoResponse.success && holdingStrategies && holdingStrategies.success && (
        <div className="finance-card finance-animate-in">
          <div className="finance-card-header">
            <h3 className="finance-card-title">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="finance-icon-lg">
              ...
              当前持仓
              <span className="finance-badge">{holdingStrategies.data.length}</span>
            </h3>
          </div>
          <div>
                  ...
                  <table className="finance-table">
                    <thead>
                      <tr>
                        ...
                        <th className="text-right">仓位占比</th>
                        <th className="text-right">持仓成本</th>
                        <th className="text-right">当前价格</th>
                        <th className="text-right">持仓盈亏</th>
                        <th className="text-right">备注</th>
                        <th className="text-right">最近更新</th>
                      </tr>
                    </thead>
                    <tbody>
                      {holdingStrategies.data.map((strategy, index) => {
                        ...
                        const marketValue = strategy["市值"];
                        
                        const profit = marketValue - entryPrice;
                        ...
                        return (
                          <tr key={index}>
                            <td>{strategy["策略"] || '-'}</td>
                            <td><strong>{strategy["标的"] || '-'}</strong></td>
                            ...
                            <td className="text-right font-medium">${entryPrice ? entryPrice.toLocaleString(undefined, {maximumFractionDigits: 2}) : "-"}</td>
                      ...
                      <div key={index} className="finance-card p-3">
                        ...
                        <div className="flex justify-between items-center border-b pb-2 mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">{strategy["策略"] || 'STAKING'}</span>
                          </div>
                          <div>
                            <strong className="text-lg">{strategy["标的"] || '-'}</strong>
                          </div>
                        </div>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                              ...
                              <div className="flex items-center justify-end">
                                <span>{(proportion * 100).toFixed(2)}%</span>
                                <div className="finance-progress-bar w-16 ml-2">
                                  <div className="finance-progress-value" style={{width: `${Math.min(proportion * 100, 100)}%`}}></div>
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="flex justify-between mb-3">
                          <div>
                            ...
                            <div className="font-medium">${entryPrice ? entryPrice.toLocaleString(undefined, {maximumFractionDigits: 0}) : "-"}</div>
                          </div>
                          <div className="text-right">
                            <div className="flex items-center justify-end gap-1 text-sm text-muted-foreground">
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className={`text-right ${profit >= 0 ? "text-green-600" : "text-red-600"}`}>
                            <span className="text-lg font-medium flex items-center justify-end gap-1">
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                {profit >= 0 ? (
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941" />
                                ) : (
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.25 6L9 12.75l4.286-4.286a11.948 11.948 0 014.306 6.43l2.199 6.43m-13.608-2.83l8.807-8.807m-8.807 8.807A11.953 11.953 0 012.25 6z" />
                                )}
                              </svg>
